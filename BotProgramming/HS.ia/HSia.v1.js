...
const zooDeck = {
    ...
    value: (card, turn) => {
        ...
        let value =
            ((card.attack * attack + card.defense * defense) / card.cost) *
            cost;
        value *= Object.keys(abilities).reduce(
            (total, ability) =>
                total * (card[ability] ? abilities[ability] : 1),
            1
        );
        value *= card.attack !== 0;
        return value;
...
class Board extends Array {
    ...
    sortBy(carac, asc) {
        return this.sort(
            (prev, next) =>
                asc ? prev[carac] - next[carac] : next[carac] - prev[carac]
        );
    }
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
while (true) {
    ...
    if (phase === "DRAFT") {
        ...
        const opponentBoard = new Board(
            ...cards.filter(card => card.location === -1)
        );
        const actions = [
...
function draft(cards, turn) {
    const pickBestCard = cards =>
        cards.sort((prev, next) => next.value - prev.value)[0];

    const pickCurvedCard = cards => {
        const deck = card => card.deck();

        const sortedByBest = cards.sort(
        ...
        if (!chosenCard) {
            chosenCard = sortedByBest.find(
                card => deck(card)[card.cardType] > 0
            );
        ...
        log(
            `${cardToPick.attack} ${cardToPick.defense} pour ${cardToPick.cost}`
        );
        log(deck(cardToPick));
        return cardToPick;
    };
...
function summonCreatures(myHand, myBoard) {
    function dfs(graph, root, cards, totalMana, paths) {
        ...
        for (let i = 0; i < newGraph.length; i++) {
            ...
            if (
                newCost <= me.playerMana &&
                myBoard.length + cards.length <= 5
            ) {
    ...
    const sortedByCost = myHand
        .filter(
            card => card.cardType === "creature" && card.cost <= me.playerMana
        )
...
function attackOpponent(myBoard, opponentBoard) {
    ...
    opponentBoard.length &&
        actions.push(...lethalsAttack(myBoard, opponentBoard));
    ...
    cantKillPlayer(myBoard) && actions.push(...attack(myBoard, opponentBoard));
...
function attackGuards(myBoard, opGuards) {
    function dfs(opGuard, graph, root, combo, totalAttack, combos) {
        ...
        if (newGraph.length === 0) {
            ...
            combos.push({
                ...
                loss: combo.reduce(
                    (total, card) =>
                        total + (card.defense - opGuard.attack <= 0 ? 1 : 0),
                    0
                ),
                efficient: combo.reduce(
            ...
            if (totalAttack < opGuard.defense) {
                ...
                combos.push({
                    ...
                    loss: combo.reduce(
                        (total, card) =>
                            total +
                            (card.defense - opGuard.attack <= 0 ? 1 : 0),
                        0
                    ),
                    efficient: combo.reduce(
                        (total, card) => total - card.attack,
    ...
    function chooseBestCombo(combos, opGuards) {
        const chosenCombos = [];
        for (let i = 0; i < opGuards.length; i++) {
            let opGuardCombos = combos.filter(
                combo =>
                    !combo.opGuard.downed &&
                    !combo.combo.some(card => card.used)
            );
            ...
            if (freeTrades.length === 0) {
                const killingCombos = opGuardCombos.filter(
                    combo => combo.efficient <= 0
                );
                const sortedByLoss = (killingCombos.length === 0
                    ? opGuardCombos
                    : killingCombos
            ...
            bestCombos = sorted
                ...
                .sort(
                    (prev, next) => next.opGuard.attack - prev.opGuard.attack
                );
    ...
    const chosenCombos = chooseBestCombo(combos, opGuards);
    actions.push(
        ...chosenCombos.map(combo =>
            combo.combo
                .map(card => `ATTACK ${card.id} ${combo.opGuard.id}`)
                .join(";")
        )
    );
...
function cantKillPlayer(myBoard) {
    const totalAttack = myBoard
        .filter(card => !card.used)
        .reduce((total, card) => card.attack + total, 0);
    return ennemy.playerHealth > totalAttack;
}

function attack(myBoard, opponentBoard) {
    function dfs(op, graph, root, combo, totalAttack, combos) {
        const newGraph = graph.filter(card => card.id !== root.id);
        if (newGraph.length === 0) {
            //lorsqu'on atteint le bout du graph
            combos.push({
                ...
                loss: combo.reduce(
                    (total, card) =>
                        total + (card.defense - op.attack <= 0 ? 1 : 0),
                    0
                ),
                efficient: combo.reduce(
                    (total, card) => total - card.attack,
            ...
            if (totalAttack < op.defense) {
                ...
                combos.push({
                    ...
                    loss: combo.reduce(
                        (total, card) =>
                            total +
                            (card.defense - op.attack <= 0 || op.lethal
                                ? 1
                                : 0),
                        0
                    ),
    ...
    function chooseBestCombo(combos, opponents) {
        ...
        for (let i = 0; i < opponents.length; i++) {
            let opCombos = combos.filter(
                combo =>
                    !combo.op.downed && !combo.combo.some(card => card.used)
            );
            //freetrades
            let bestCombos,
                sorted = [];
            ...
            const efficientsTrades = killingCombos
                .filter(
                    combo =>
                        ...
                        combo.loss < 2 &&
                        combo.op.defense - combo.efficient <= combo.op.attack
                )
                .sort((prev, next) => next.efficient - prev.efficient);
            ...
            bestCombos = sorted
                .filter(
                    combo =>
                        combo.loss === sorted[0].loss &&
                        combo.efficient === sorted[0].efficient
                )
                .sort((prev, next) => next.op.attack - prev.op.attack);

            bestCombos.forEach(combo => {
                if (!combo.combo.some(card => card.used)) {
                    //eviter de push 2 fois le même combo
                    chosenCombos.push(combo);
                    combo.op.downed = true;
                    combo.combo.forEach(card => {
                        card.used = true;
                    });
                }
    ...
    const myUnusedBoard = myBoard.filter(card => !card.used);
    const opponents = opponentBoard.filter(opponent => !opponent.guard);
    const actions = [],
        combos = [];
    opponents.forEach(opponent => {
        for (let i = 0; i < myUnusedBoard.length; i++) {
            const root = myUnusedBoard[i];
            dfs(opponent, myUnusedBoard, root, [root], root.attack, combos);
        }
    });

    const chosenCombos = chooseBestCombo(combos, opponents);
    actions.push(
        ...chosenCombos.map(combo =>
            combo.combo
                .map(card => `ATTACK ${card.id} ${combo.op.id}`)
                .join(";")
        )
    );
...
function lethalsAttack(myBoard, opponentBoard) {
    ...
    myBoard.forEach(card => {
        if (card.lethal) {
            const opponent = (opGuards.length
                ? opGuards
                : opponentBoard
            ).reduce(
                (chosen, opponent) =>
                    chosen.defense > opponent.defense ? chosen : opponent
            );
            card.used = true;
    ...
    opponents.forEach(opponent => {
        if (opponent.ward) {
            const weakest = myBoard.reduce(
                (weakest, card) =>
                    weakest.attack < card.attack && !weakest.used
                        ? weakest
                        : card
            );
            weakest.used = true;
