...
const zooDeck = {
    0: 0, 1: 4, 2: 4, 3: 6, 4: 6, 5: 5, 6: 3, 7: 2, 8: 1, 9: 0, 10: 1, 11: 0, 12: 0,
    'creature': 30,
    value: (card, turn) => {
        const attack = 3;
        const defense = 2;
...
let me = {};
let ennemy = {};
let opponentCard = new Card({ id: -1, attack: 0, defense: 30, abilities: '------' });
let phase = 'DRAFT';
...
function getAttackerOpponent(myCard, myBoard, opponentBoard) {
    return opponentBoard.reduce((chosenCard, card) => {
        ...
        if (myCard.lethal) {
            ...
            if ((chosenCard.id == -1 && card.attack + card.defense > 6) || (card.attack * 2 + card.defense > chosenCard.attack * 2 + chosenCard.defense)) {
                return card;
            }
        }
        ...
        if (hasNoGuard(myBoard) && myCard.attack >= card.defense && myCard.defense > card.attack) {
            return card;
        }
...
function updatePlayersAndCurrentPhase() {
    const getPlayerInfo = () => {
        const [playerHealth, playerMana, playerDeck, playerRune] = readline().split(' ').map(val => parseInt(val));
...
function tree(graph, cards, totalMana, index) {
    const nextCard = graph[index];
    const newCost = totalMana + nextCard.cost;
    const newGraph = graph.filter(card => card.id !== nextCard.id);
    if(newCost > me.playerMana) {
        return cards;
    }
    cards.push(nextCard);
    return tree(newGraph, cards, newCost, index);
}
...
function summonCreatures(myHand, myBoard) {
    ...
    for(let i = 0; i < sortedByCost.length; i++) {
        ...
        for(let index = 0; index < sortedByCost.length - 1; index++) {
            const combo = tree(sortedByCost, [root], root.cost, index);
            log('combo', combo.map(card => ({mana: card.cost, id: card.id})));
        }
